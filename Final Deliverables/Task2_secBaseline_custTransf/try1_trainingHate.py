...
def generate_advanced_label(row):
    """Convert label to binary format (0 for not hate, 1 for hate)"""
    # Check the type of label in your CSV files
    try:
        # If label is numeric
        label = int(row['label'])
        # Assuming 0 = not hate, 1 = hate
        return label
    except:
        # If label is text
        label_text = str(row['label']).lower()
        if label_text in ['hate', 'hate_speech', '1', 'true', 'yes']:
            return 1  # Hate
        elif label_text in ['not_hate', 'normal', '0', 'false', 'no']:
            return 0  # Not hate
        else:
            # If label format is unknown, return None
            print(f"Unknown label format: {row['label']}")
            return None
...
def main():
    ...
    model = TransformerWithRestrictedAttention(
        ...
        max_len=max_len,
        num_classes=2,
        n_heads=12,  # Need to be divisible by embed_size (768)
        num_encoder_layers=6,
        dim_feedforward=3072,
        dropout=0.2,
        window_size=15  # Restricted context window size
    ).to(device)
    ...
    # Plot confusion matrix
    plot_confusion_matrix(all_labels, all_preds, ['Not Hate', 'Hate'], fold_number="Final Model")
    
    # Plot training history
    plot_history(history, fold_number="Training History")
...
def inference():
    ...
    checkpoint_path = 'final_model.pt'  # Change to your best performing model
